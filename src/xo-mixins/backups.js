...
export default class Backups {
  ...
  async deltaCopyVm ($onFailure, $cancelToken, srcVm, targetSr, force = false, retention = 1) {
  ...
  @cancelable
  @deferrable.onFailure
  async rollingDeltaVmBackup ($onFailure, $cancelToken, {vm, remoteId, tag, retention}) {
    const handler = await this._xo.getRemoteHandler(remoteId)
  ...
  @cancelable
  async backupVm ($cancelToken, {vm, remoteId, file, compress, onlyMetadata}) {
    const handler = await this._xo.getRemoteHandler(remoteId)
    return this._backupVm($cancelToken, vm, handler, file, {compress, onlyMetadata})
  }

  async _backupVm ($cancelToken, vm, handler, file, {compress, onlyMetadata}) {
  ...
  @cancelable
  async rollingBackupVm ($cancelToken, {vm, remoteId, tag, retention, compress, onlyMetadata}) {
    const handler = await this._xo.getRemoteHandler(remoteId)
    ...
    const data = await this._backupVm($cancelToken, vm, handler, file, {compress, onlyMetadata})
